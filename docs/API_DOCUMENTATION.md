...
В backend также остаются старые локальные роуты `/api/auth/*` и `/api/scan/*`. Они нужны для совместимости и локальных экспериментов, но мобильное приложение в целевой схеме должно использовать только `/api/v1/*`.
